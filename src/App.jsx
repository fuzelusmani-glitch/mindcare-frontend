    ...
    <main className="max-w-6xl mx-auto px-4 py-8">
      <section className="rounded-2xl bg-gradient-to-br from-white to-emerald-50 dark:from-slate-800 p-8 shadow">
        ...
        <p className="mt-3 text-sm opacity-80">
          AI-powered empathetic support, mood tracking, and easy access to professionals — private and stigma-free.
        </p>
        ...
        <form onSubmit={handleSubmit} className="mt-4 space-y-3" aria-label="login form">
          ...
          <label className="block text-sm">
            ...
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
              className="mt-1 w-full px-3 py-2 rounded-md border focus:ring-2 focus:ring-emerald-200" required />
          </label>
          <label className="block text-sm">
            <span className="text-xs">Password</span>
            <input type="password" value={password} onChange={(e) => setPassword(e.target.value)}
              className="mt-1 w-full px-3 py-2 rounded-md border" required />
          </label>
          <div className="flex items-center justify-between">
            <button className="px-4 py-2 bg-emerald-600 text-white rounded-md">Sign in</button>
            <Link to="/signup" className="text-sm underline">Create an account</Link>
          </div>
        </form>
...
function Dashboard({ user }) {
  ...
  async function handleSend() {
    ...
    try {
      ...
      const res = await API.post("/chat", { messages: payloadMessages }); // ✅ fixed
      const aiText = res.data.reply || "Sorry, something went wrong.";
      setMessages((m) => [...m, {
    ...
    <div className="max-w-7xl mx-auto px-4 py-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
      <section className="lg:col-span-4 bg-white dark:bg-slate-800 rounded-2xl shadow p-4 flex flex-col h-[70vh]" aria-labelledby="chat-heading">
        <div className="flex items-start justify-between">
          <div>
            <h2 id="chat-heading" className="text-lg font-semibold">Talk to MindCare</h2>
            ...
            {messages.map((msg) => (
              <motion.div key={msg.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div className={`${msg.from === "user"
                  ? "bg-emerald-600 text-white rounded-lg rounded-br-sm"
                  : "bg-slate-100 dark:bg-slate-700 text-slate-900 dark:text-slate-100 rounded-lg rounded-bl-sm"
                } px-4 py-2 max-w-[85%] break-words shadow-sm`}>
        ...
        <div className="mt-3">
          <div className="flex items-center gap-3">
            <input ref={inputRef} aria-label="Type your message" value={input}
            ...
            <button onClick={handleSend} aria-label="Send message"
              className="p-2 rounded-lg bg-emerald-600 text-white hover:brightness-95 focus:ring-2 focus:ring-emerald-300">
              <Send />
            </button>
          </div>
      ...
      <section className="lg:col-span-5 bg-white dark:bg-slate-800 rounded-2xl shadow p-4 h-[70vh] flex flex-col" aria-labelledby="dashboard-heading">
        <div className="flex items-start justify-between">
          <div>
            <h2 id="dashboard-heading" className="text-lg font-semibold">Mood Dashboard</h2>
          ...
          <div className="flex items-center gap-3">
            ...
            <select value={selectedMetric} onChange={(e) => setSelectedMetric(e.target.value)}
              className="rounded-md p-1 bg-slate-50 dark:bg-slate-700">
        ...
        <div className="mt-4 flex-1 grid grid-rows-2 gap-4">
          <div className="row-span-1 rounded-lg p-3 bg-gradient-to-br from-white to-emerald-50 dark:from-slate-800 dark:to-slate-800/60 shadow-sm">
            <div className="flex items-center justify-between">
              ...
              <div className="text-xs opacity-75"><p>Interactive • Hover or tap points to see details</p></div>
            </div>
            ...
            <div className="rounded-lg p-3 bg-white dark:bg-slate-700 shadow-sm flex flex-col justify-between">
              <div>
                ...
                <ul className="mt-2 space-y-2 text-xs">
                  <li>• Average mood this month: <strong>{calcAvgMood(moodData).toFixed(2)}</strong></li>
                  <li>• Most frequent feeling: <strong>{mostFrequentMood(moodData)}</strong></li>
                  <li>• Recorded entries: <strong>{moodData.length}</strong></li>
                </ul>
              </div>
      ...
      <section className="lg:col-span-3 bg-white dark:bg-slate-800 rounded-2xl shadow p-4 h-[70vh] flex flex-col" aria-labelledby="locator-heading">
        <div className="flex items-start justify-between">
          <div>
            <h2 id="locator-heading" className="text-lg font-semibold">Find a Provider</h2>
            <p className="text-xs opacity-80">Locate nearby mental health professionals and quick-book a call.</p>
          </div>
          <button className="text-xs opacity-80 underline" onClick={() => alert("Open filter modal (integration)")}>Filters</button>
        </div>
        <div className="mt-3 flex-1 flex flex-col gap-3">
          ...
          <div className="overflow-auto space-y-3">
            {nearbyDoctors.map((doc) => (
              <article key={doc.id} tabIndex={0} role="button" onClick={() => setSelectedDoctor(doc)}
                className="p-3 rounded-lg border hover:shadow-md focus:outline-none focus:ring-2 focus:ring-emerald-200">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-medium">{doc.name}</h3>
                    <p className="text-xs opacity-75">{doc.specialty} • {doc.distance} km</p>
                  </div>
          ...
          <div className="mt-2">
            <button className="w-full rounded-lg py-2 bg-emerald-600 text-white shadow"
              onClick={() => alert("Open booking flow (integration)")}>Book a Session</button>
          </div>
        </div>
      </section>
    </div>
      ...
      <div className="mt-3 flex gap-2">
        {doctors.slice(0, 3).map((d) => (
          <button key={d.id} onClick={() => onSelect(d)}
            className="px-2 py-1 rounded-md bg-white dark:bg-slate-700 border text-xs">
            {d.name.split(" ")[0]}
          </button>
        ))}
      </div>
...
function calcAvgMood(data) {
  ...
  return data.reduce((a, b) => a + b.mood, 0) / data.length;
}
...
export default function App() {
  ...
  function handleLogout() { localStorage.removeItem("mindcare_user"); setUser(null); }
      ...
      <div className={`min-h-screen antialiased ${darkMode ? "bg-slate-900 text-slate-100" : "bg-gradient-to-b from-sky-50 via-emerald-50 to-white text-slate-900"}`}>
        ...
        <footer className="max-w-7xl mx-auto px-4 py-4 text-xs text-center opacity-80">
          <div className="flex items-center justify-center gap-3">
            ...
            <button onClick={() => setShowOnboarding(true)} className="underline">Privacy settings</button>
          </div>
        </footer>
        <AnimatePresence>
          {showOnboarding && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <motion.div initial={{ y: 20, scale: 0.99 }} animate={{ y: 0, scale: 1 }} exit={{ y: 20, opacity: 0 }}
                className="relative w-full max-w-2xl bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-lg">
                <div className="flex items-start gap-4">
                  <div className="rounded-full p-3 bg-emerald-100 dark:bg-emerald-900/30"><Heart /></div>
                  <div>
                    ...
                    <div className="mt-4 flex items-center gap-3">
                      <button onClick={() => setShowOnboarding(false)} className="px-4 py-2 rounded-lg bg-emerald-600 text-white">Got it</button>
                    </div>
